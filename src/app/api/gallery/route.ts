...
function isDummyOrTestItem(id: string, builderName?: string): boolean {
  ...
  // Exclude test fixture IDs generated by vitest unit test suites
  if (
...
export async function GET() {
  ...
  // 1. Fetch user-generated graphics from Supabase Storage
  if (supabase) {
    try {
      // Cards from Supabase
        ...
        for (const file of cardFiles) {
          if (file.name.endsWith('.png')) {
            ...
            try {
              ...
              // Ignore single metadata download error
            }
      ...
      console.warn('Gallery Supabase listing warning:', err);
    }
  }

  // 2. Fallback / Merge with server memory & disk records
  try {
    ...
    for (const rec of localRecords) {
      ...
      if (!itemsMap.has(rec.id)) {
        itemsMap.set(rec.id, {
          id: rec.id,
          type: rec.type,
          imageUrl: rec.publicUrl || rec.imageDataUrl || `/api/og?id=${rec.id}&type=${rec.type}`,
          shareUrl: rec.shareUrl || `/${rec.type}/${rec.id}`,
          createdAt: rec.createdAt,
          builderName: rec.builderInfo?.name,
          builderTitle: rec.builderInfo?.builderTitle,
          builderRole: rec.builderInfo?.role,
          builderCompany: rec.builderInfo?.company || rec.builderInfo?.college,
        });
      }
    }
  } catch (err) {
